...
problem, and then multiplying the result by base to solve the 
initial problem.

This function should take in two values - base can be a float or 
an integer; exp will be an integer ≥0. It should return one 
...
def recurPower(base, exp):
    ...
    if(abs(exp) > 1):
        if expSign > 0:
            return base * recurPower(base, abs(exp) - 1)
        else:
            return 1.0 / (base * recurPower(base, abs(exp) - 1))
    return base
